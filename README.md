 # Luiz Felipe Olivieri — QA Engineer

  Analista de Testes com 1 ano de experiência em projetos ágeis,
  atuando com testes manuais, automação e validação de APIs.
  Inglês fluente | São Paulo, BR

  ---

  ## Stack

  - **Automação:** Robot Framework · Playwright · Appium · Selenium
  - **API:** Postman · RequestsLibrary
  - **Metodologias:** BDD/Gherkin · Scrum · Kanban

  ---

  ## Foco atual

  Aprofundando automação com Robot Framework e Playwright,
  boas práticas de CI e cobertura de testes de API.

  ---

  ## Contato

  [![LinkedIn](https://img.shields.io/badge/LinkedIn-luizolivieriqa-blue?logo=linkedin)](https://linkedin.com/in/luizolivieriqa

  ---
  
  ## Stack

  [![Skills](https://skillicons.dev/icons?i=js,html,css,postman,git)](https://skillicons.dev)

  ![Robot
  Framework](https://img.shields.io/badge/Robot_Framework-000000?style=for-the-badge&logo=robot-framework&logoColor=white)
  ![Selenium](https://img.shields.io/badge/Selenium-43B02A?style=for-the-badge&logo=selenium&logoColor=white)
  ![Playwright](https://img.shields.io/badge/Playwright-2EAD33?style=for-the-badge&logo=playwright&logoColor=white)
  ![Appium](https://img.shields.io/badge/Appium-40C4FF?style=for-the-badge&logo=appium&logoColor=white)
  ![Postman](https://img.shields.io/badge/Postman-FF6C37?style=for-the-badge&logo=postman&logoColor=white)
